...
export const OTapproved = () => {
  const [request, setRequest] = useState([]);

  //fetching approved request per user
  const getRequest = async () => {
    try {
      const res = await fetch(
        "https://otnsbackend.vercel.app/api/request/approve/",
        {
          method: "GET",
  ...
  useEffect(() => {
    const initializeDataTable = () => {
      ...
      if (request.length > 0 && !$.fn.DataTable.isDataTable("#example2")) {
    ...
    <div className="main-container">
      <div className="history-container">
        <Container className="history align-items-center" id="approved">
          ...
          <table id="example2" className="table table-striped  nowrap bg-dark ">
            <thead>
