...
export const OTrejected = () => {
  const [request, setRequest] = useState([]);

  //fetching rejected request per user

  const getRequest = async () => {
    try {
      const res = await fetch(
        "https://otnsbackend.vercel.app/api/request/reject/",
        {
          method: "GET",
  ...
  useEffect(() => {
    const initializeDataTable = () => {
      ...
      if (request.length > 0 && !$.fn.DataTable.isDataTable("#rejected")) {
